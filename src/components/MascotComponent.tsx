...
export const MascotComponent: React.FC<MascotComponentProps> = ({ currentPage, lastAction, voiceType }) => {
  const [speech, setSpeech] = useState<string>("哈罗！我是五音守护灵福狸酱，今天我们也要努力熟化拼音哦！");
  ...
  useEffect(() => {
    ...
    if (voiceType === "male") {
      setSpeech("已切换为沉厚博雅的【男声导师】！雄浑有磁性，快去敲打拼读听听吧！");
      setExpression("determined");
    } else if (voiceType === "child") {
      setSpeech("哇塞！你选了软萌的【童声伴读】！太清脆好听啦，听着像我的小伙伴！");
      setExpression("celebrate");
    } else if (voiceType === "alien") {
      setSpeech("哔哔……检测到跨纬度宇宙的【外星人声】！音调居然如此魔性高亢，难道是狸狸母星的赛博来客？！");
      setExpression("shocked");
    } else if (voiceType === "elderly") {
      setSpeech("已切换为宽仁和蔼的【智慧老人】！温热迟缓的声音里满是岁月沉淀，非常有韵味哦~");
      setExpression("happy");
    } else {
      setSpeech("已切回亲切温婉的【女声导师】！风吟拂面，标准和风朗读最配你啦。");
      setExpression("happy");
    }
  }, [voiceType]);
  ...
  useEffect(() => {
    ...
    if (currentPage === "start") {
      ...
      setSpeech("打起精神！敲击对应健位，心手合一，让指尖起舞吧！加油！");
      setExpression("determined");
  ...
  useEffect(() => {
    if (currentPage === "training" && lastAction) {
      if (lastAction === "correct") {
        ...
        setExpression("celebrate");
        // Trigger small celebration jump
        setTimeout(() => setExpression("happy"), 1500);
      }
    }
  }, [lastAction, currentPage]);
  ...
  const handleMascotClick = () => {
    ...
    // Quick synth beep response from mascot
    try {
    ...
    <div className="fixed bottom-4 right-4 z-40 flex flex-col items-end pointer-events-none select-none">
      ...
      <AnimatePresence>
        {showBubble && (
          <motion.div
            ...
            className="mb-2 max-w-[200px] bg-white text-stone-900 border-2 border-amber-500 rounded-2xl p-3 shadow-xl pointer-events-auto relative cursor-pointer"
          ...
          >
            ...
            <div className="text-[11px] font-sans leading-relaxed text-stone-800 flex flex-col gap-1">
              <span className="font-bold text-amber-700 flex items-center justify-between text-[10px] font-mono tracking-wider border-b border-stone-100 pb-0.5">
                <span>TSUKIMI FOX</span>
                <span className="flex items-center gap-0.5 text-rose-500"><Heart className="w-2.5 h-2.5 fill-current" /> MASCOT</span>
              </span>
              <span>{speech}</span>
              <span className="text-[8px] text-stone-400 text-right mt-1 font-mono">点击对话框可关闭</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Raccoon/Fox Character Avatar Container with dynamic floating physics */}
      <motion.div
        animate={{
          y: [0, -6, 0],
        }}
        ...
        className="pointer-events-auto cursor-pointer relative group flex flex-col items-center justify-end"
      ...
      >
        ...
        <svg
          ...
          height="88"
          viewBox="0 0 100 100"
          className="drop-shadow-lg transform active:scale-95 transition-transform"
        >
          ...
          {/* White cheeks circles to make it look extremely Japanese cute mascot style */}
